...
const toggleFabAndOverlay = ()=>{
    console.log('',appStore.overlay)
    appStore.toggleOverlay();
    appStore.toggleFabMenu();
}
...
const actions = [
 ...
 const FabButton = observer(() => {
    return (
        <Box sx={{height: 330, transform: 'translateZ(0px)', position: 'absolute', top: 1, right: 1, zIndex: 100}}>
            <Backdrop open={appStore.fabMenu} hidden={true}/>
            <SpeedDial
                ariaLabel="Настройки"
                direction='down'
                sx={{position: 'absolute', top: 32, right: 32, zIndex: 101}}
                icon={
                    <SvgIcon>
                        <svg width="60px" height="60px" viewBox="0 0 1024 1024" class="icon gear-icon"
                ...
                onClose={toggleFabAndOverlay}
                onOpen={toggleFabAndOverlay}
                open={appStore.fabMenu}>
                {actions.map((action) => (
                    <SpeedDialAction
                        key={action.name}
                        icon={action.icon}
